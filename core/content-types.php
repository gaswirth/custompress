	...
	class CustomPress_Content_Types extends CustomPress_Core {
		...
		function __construct() {
			...
			add_action( 'init', array(
				&$this,
				'flush_rewrite_rules'
			), 99 ); //Give everyone else a chance to set rules, endpoints etc.
		...
		function display_custom_fields( $fields = '', $style = '' ) {
			$this->render_admin( 'display-custom-fields', array(
				'type'   => 'local',
				'fields' => $fields,
				'style'  => $style
			) );
				...
				switch ( $property ) {
					...
					default: {
						switch ( $custom_field['field_type'] ) {
							...
							default: {
								$result = get_post_meta( $post->ID, $id, true );
								break;
							}
						}
					}
						...
						case 'upload':
							...
							if ( count( $meta ) ) {
								...
								if ( wp_attachment_is_image( $meta[0] ) ) {
									...
									$select  = '<select style="display:block" class="ct-upload-image-size">';
									foreach ( $options as $k => $option ) {
										$select .= sprintf( '<option ' . selected( $meta[1], $k, false ) . ' value="%s">%s</option>', $k, $option );
									}
									$select .= '</select>';
									$result .= $select;
								}
		...
		function validation_rules( $atts, $content = null ) {
			...
			$validation[] = "jQuery('[id^={$selector}]').closest('form').validate({ignore:[]});"; //find the form we're validating
			...
			foreach ( $custom_fields as $custom_field ) {
				...
				if ( ! empty( $regex_message ) ) {
					$msgs[] = "regex: '{$regex_message}'";
				}


				if ( ! empty( $msgs ) ) {
					$validation[] = "jQuery('[name={$fid}]').rules('add', { messages: {" . implode( ", ", $msgs ) . " } });";
				}

				//Collect rules
				$rls = array();
				if ( $custom_field['field_required'] || ! empty( $custom_field['field_regex'] ) ) { //we have validation rules
					if ( ! empty( $custom_field['field_required'] ) ) {
						$rls[] = 'required: true';
					}
				...
				if ( ! empty( $rls ) ) {
					$validation[] = "jQuery('[name={$fid}]').rules('add', { " . implode( ", ", $rls ) . " } );";
				}
			}
		...
		function _import( $types = array() ) {
			...
			if ( ! ( is_array( $types ) && defined( 'CT_ALLOW_IMPORT' ) ) ) {
				return;
			}
